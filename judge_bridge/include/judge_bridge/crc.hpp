...
namespace JudgeBridge {
    ...
    public:
        ...
        */ 
        static uint16_t Verify_CRC8_Check_Sum(uint8_t *pchMessage, uint16_t dwLength) { 
            uint8_t ucExpected = 0;
            if ((pchMessage == 0) || (dwLength <= 2)) return 0; 
            ucExpected = VerifyCRC::Get_CRC8_Check_Sum (pchMessage, dwLength-1, CRC8_INIT); 
        ...
        ** Output: True or False (CRC Verify Result) 
        */ 
        static void Append_CRC8_Check_Sum(uint8_t *pchMessage, uint16_t dwLength) 
        { 
            uint8_t ucCRC = 0; 
            if ((pchMessage == 0) || (dwLength <= 2)) return; 
            ucCRC = VerifyCRC::Get_CRC8_Check_Sum ( (uint8_t *)pchMessage, dwLength-1, CRC8_INIT); 
        ...
        ** Input: Data to Verify (without CRC), Stream length = Data
        ** Output: True or False (CRC Verify Result) 
        */ 
        static uint32_t Verify_CRC16_Check_Sum(uint8_t *data, uint32_t dwLength) { 
            uint16_t wExpected = 0; 
            if (data == nullptr) { 
                return false; 
            } 

            wExpected = VerifyCRC::Get_CRC16_Check_Sum ( data, dwLength - 2, CRC_INIT); 
            return ((wExpected & 0xff) == data[dwLength - 2] && ((wExpected >> 8) & 0xff) == 
            data[dwLength - 1]); 
        }
        ...
        */ 
        static void Append_CRC16_Check_Sum(uint8_t * pchMessage,uint32_t dwLength) { 
            ...
            wCRC = VerifyCRC::Get_CRC16_Check_Sum ( (uint8_t *)pchMessage, dwLength-2, CRC_INIT ); 
